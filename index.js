...
app.get("/quiz/:id", async function(req, res) {
  ...
  const [quiz, submissions] = await Promise.all([
      db.collection('quiz').findOne({ _id: id }),
      db.collection('submission').find({ quiz_id: id }).toArray()
  ]);

  res.status(200).json( {quiz_id: id, submissions: submissions })
});
...
// get quiz and all its submissions 
// app.get("/quiz/:id", async function(req, res) {
//   var id = req.params.id 
...
//   res.status(200).json( {quiz: quiz, submissions: submissions })
// })
